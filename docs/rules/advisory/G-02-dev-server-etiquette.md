...
- **夜間帯はコマが0件になる**: `config/polling.yaml`の20:00〜04:00は、取得が止まるだけでなく、保存済みのコマも返らない(`catalogAccess.allowed=false`、`reason=scheduled_stopped`)。夜間に実機で確認するには、同ファイルの該当期間をローカルで日中並みに変え、APIを再起動する。このファイルは追跡対象で、ローダーに既定値フォールバックがないため、追跡から外してはいけない。ローカル変更が混ざるので、`git add`は必ずパスを指定し(`-A`・`.`を使わない)、コミットしない。
