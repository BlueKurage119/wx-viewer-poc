...
6. **配色は`apps/web/src/theme/`が生成する`--md-sys-color-*`のみを使う。** シード色は`apps/web/src/theme/seeds.ts`の`DEFAULT_THEME_SEED`(`#1A73E8`)のみで完結させる。共通シェルはダーク固定(README参照)。警戒レベル色・通知区分色はこのMD3基盤の対象外で、別途セマンティックトークンとして定義する(basic-design.md §5.7参照、未着手)。
...
1. 凡例用データ色トークンとして、`--wx-data-nowcast-1`の例により定義する。
