...
- [ ] AC12: sourceVersionをInfoKindVersionに戻すと訂正識別テストが失敗すること、会場または通知種別を識別から落とすと件数テストが失敗することを対照実験で確認。通常notificationIdと起動outputIdが監査・履歴で混同されない。
- [ ] AC13: 履歴保存失敗を注入し、現況保存は維持され、成功件数を誤報せず、同版再取得による再送保証を追加していないことを確認。
- [ ] AC14: `npm run lint`、`npm run typecheck`、`npm run format:check`、`npm run test -w apps/api`、`npm run test -w packages/shared`および起動型の利用箇所に影響があればwebテストを実行して全件成功。
