...
| 切替時の挙動 | 地図中心・ズームを保持、再生停止、**切替前に選択していた時刻を維持** |
| 表示窓 | 取得済みフレームのうち**過去 3 時間**のみをクライアント側で表示 |
| 凡例 | 公式の階級区分・名称・配色で実仕様化し、選択中の種別に応じて差し替え。データ色トークン `--wx-data-kikikuru-*` を定義 |
| 時刻表現 | 危険度判定の基準時刻としての表示。実況／予測と表現しない |
...
`apps/web/src/map/LayerSelector.tsx` は**変更しない**（§5.4）。`apps/web/src/map/types.ts` の既存型、`packages/shared` の DTO も変更しない。
...
/** 表示窓（過去 3 時間）を適用したうえで TimelineFrame へ変換する（§6.4） */
...
3. **`selectedFrameId` を変更しない**（§5.3）。ただし切替先に同じ `validTime` のコマが存在しない場合の扱いは §5.3 に従う。
...
### 5.3 時刻の引き継ぎ（2026-09-15 確定）

Issue #46 の受け入れ条件「切り替え直後は再生停止・最新時刻表示になる」を、統括担当の判断により次へ**修正する**。

> **切り替え直後は再生を停止し、切替前に選択していた時刻を維持して表示する。**

根拠は §3.1 の実測である。キキクル 3 種は同一の `targetTimes.json` に由来し、`elements` に `rain_mesh` / `inund` / `land` が 37 件すべてで同時に含まれるため、**3 種の時間軸は完全に一致する**。大雨→浸水の切替で時刻をリセットすると、ユーザーが調べていた時点の別種別を見るという最も自然な操作が毎回巻き戻ることになる。

実装規約:

- `selectedFrameId` は `validTime` のみから生成するため（§6.3）、種別が変わっても id はそのまま通用する。切替時に `selectedFrameId` を書き換えない。
- 切替先に同じ `validTime` のコマが存在しない場合（索引の availability 差などで起こりうる）、**黙って別時刻へ差し替えない**。`selectedFrameId` を `null` にし、§6.3 と同じ「提供範囲外」の案内を出す。
- ナウキャスト↔キキクルの切替では時間軸が一致しない（雨雲は 5 分刻み・未来コマあり、キキクルは 10 分刻み・未来コマなし）。この場合は引き継がず、**切替先の利用可能コマのうち `validTime` が最大のもの**を選ぶ。コマが 1 件もなければ `null`。
...
- 索引更新後、`selectedFrameId` が新しい一覧に存在すればそのまま維持する（`member` だけが変わっても選択は動かさない）。存在しなければ、選択を黙って別時刻へ差し替えず、`selectedFrameId` を `null` にしたうえで「選択していた時刻は提供範囲外になりました」と示し、最新時刻への復帰操作を案内する（基本設計 §4.3「別時刻へ無表示で置き換えない」）。案内文言の最終形は F7/F8 が所有するため、本書では `status` slot へ短い注記を出すに留める。
- 索引更新でフレーム一覧が変わっても、`playing === true` の間は一覧を差し替えない（基本設計 §4.3「再生中のコマ一覧は固定」）。再生タイマー自体は F7 の責務だが、**一覧固定の受け口**として、`playing` 中に受信した索引はバッファへ退避し `playing` が `false` になった時点で反映する仕組みを本書で用意する。

### 6.4 表示窓（過去 3 時間）【確定・2026-09-15】

基本設計 §4.3 が【未確定】の案として残していた「キキクルは最新を基本とし、取得可能な過去 3 時間を表示窓とする」を、統括担当の判断により**確定事項として採用する**。初版設計の「API が返した全フレーム（実データで約 6 時間・37 コマ）を表示する」暫定方針から変更する。

**フィルタはクライアント側で行う。** API (`/api/weather/kikikuru/times`) は窓を適用せず全フレームを返す契約であり（`docs/design/issue-39-nowcast-kikikuru-rest-apis.md` §3.3「雨雲の ±60 分窓や基本設計の未確定 3 時間窓を適用しない」）、`docs/design/issue-21-kikikuru-tiles.md` も「UI 用の 3 時間窓はここで勝手に適用しない」としている。**API・バックエンドを変更しない。**

実装規約:

```ts
// apps/web/src/map/kikikuru/kikikuruCatalog.ts
const DISPLAY_WINDOW_MS = 3 * 60 * 60 * 1000;

// 窓の基準は「索引が持つ最新 validTime」であり、クライアントの時計ではない
const latest = Math.max(...frames.map((f) => Date.parse(f.validTime)));
const visible = frames.filter((f) => latest - Date.parse(f.validTime) <= DISPLAY_WINDOW_MS);
```

- **窓の基準点は索引の最新 `validTime`** とする。クライアントの `Date.now()` を基準にすると、端末の時計ずれや索引の stale によって窓が最新コマを切り落としうる。最新コマを必ず含める基準とする。
- 境界は**以上・以下の閉区間**とし、ちょうど 3 時間前のコマを含める。実データの 10 分間隔では最新を含めて 19 コマになる。
- 欠けたコマを生成・補間しない（基本設計 §4.3）。窓内に存在しない時刻の枠を作らない。
- 窓を適用した結果 0 件になることはない（最新コマは必ず残る）。ただし索引そのものが空（`data.frames === []`）の場合は 0 件であり、これは §9 の「正常な空」として扱う。
- **窓の外にあるコマを「取得できていない」と表示しない。** 表示窓は画面側の都合であり、取得失敗ではない。
- 選択中の `selectedFrameId` が索引更新によって窓の外へ出た場合は、§6.3 と同じ「提供範囲外」の扱いとし、黙って別時刻へ差し替えない。
...
1. キキクルの全フレームの `TimelineFrame.kind` は **`'reference'` 固定**とする。`'observed'`（実況）・`'forecast'`（予報）を使わない。`types.ts` の `TimelineFrameKind` に `'reference'` が既にあり、F1 設計 L156 が「キキクルの危険度判定基準時刻など、実況／予報の二分法に当てはめない表示に使う」と用途を定義済みである。
2. 選択日時のラベルは「**危険度判定 基準時刻 MM/DD HH:mm**」（JST 表記）とする。「実況」「現在」「予測」「予報」「有効期限」「〜まで」の語を使わない。
3. 選択レイヤー名の近傍に、常時「**この危険度は予測を含む判定結果です**」の注記を 1 か所だけ置く。凡例内でもカード内でもよいが、重複させない。
4. `validTo` を根拠に「期限切れ」「失効」と表示しない。キキクルの時間軸に有効期限は存在しない。
5. 時間軸に未来のコマを作らない。`validTime > 現在時刻` のフレームは実データ上存在しないが、万一返っても未来コマとして特別扱いせず、他と同じ `'reference'` として並べる。
6. `aria-valuetext` にも同じ規約を適用し、スクリーンリーダーへ「実況」「予報」と読ませない。
7. 土砂キキクルを「2 時間先の予測」等の固定の先行時間で説明しない（`docs/data-acquisition-report.md` が旧資料の適用を明示的に禁じている）。
...
- [ ] `apps/web/tests` に、**キキクル種別の切替で `selectedFrameId` が変化しないこと**を検証する回帰テストが存在し、通過する（§11.4 と対になる観点）。

### 11.4 再生停止・時刻の維持（Issue 受け入れ条件 3 を修正）

**Issue #46 の受け入れ条件「切り替え直後は再生停止・最新時刻表示になる」は、§5.3 の判断（2026-09-15 統括確定）により「切り替え直後は再生を停止し、切替前に選択していた時刻を維持して表示する」へ修正されている。**
...
- [ ] キキクル表示中、スライダーに並ぶコマが**索引の最新 `validTime` から過去 3 時間以内**のものだけである。実データの 10 分間隔なら最新を含めて 19 コマになる。
- [ ] API 応答（`/api/weather/kikikuru/times`）には 3 時間より古いフレームが含まれている一方、画面のスライダーには出ていない（Network タブの応答 JSON とスライダーの目盛り数を突き合わせる）。
- [ ] 窓の外のコマについて「取得できていません」「欠測」等の失敗を示す文言が出ていない。
- [ ] 単体テストで、`toTimelineFrames` が「索引の最新 `validTime`」を基準に窓を適用し、クライアントの `Date.now()` に依存しないことを検証している（時計をずらした入力で結果が変わらないこと）。
- [ ] 単体テストで、ちょうど 3 時間前のコマが含まれる（閉区間）ことを検証している。
...
- [ ] キキクル表示中、選択日時が「危険度判定 基準時刻 MM/DD HH:mm」の形式で表示される。
- [ ] キキクル表示中の画面テキストに「実況」「予報」「予測中」「有効期限」「〜まで有効」「失効」のいずれも現れない。DOM 全体を検索して確認する。
- [ ] 「この危険度は予測を含む判定結果です」の注記が 1 か所だけ表示される（重複しない）。
...
2. **`docs/basic-design.md` §4.2 の「切り替え時は再生を停止し、切り替え先の最新の利用可能時刻を表示する案とする。時刻の引き継ぎや選択状態の保存は要協議」も更新対象である。** §5.3 のとおり、キキクル種別間では時刻を引き継ぐ方針が確定した。同じく本 Issue のスコープ外とする。
