...
| 4 | 種別切替時の時刻 | 当初は切替前の選択時刻を維持するとしたが、後続の「最新 1 コマのみ」確定により**過去時刻の引き継ぎ自体を廃止**した | §5.2、§5.3、§11.4、§18 |
...
| 6 | 表示窓の長さ | 当初は過去 3 時間としていたが、後続のオーナー指示により**最新 1 コマのみ**へ変更 | §6.4、§11.4.1、§18 |
...
| 11 | 地理院タイル出典リンク | **リンク文字色に既存 MD3 トークン `--md-sys-color-inverse-primary` を使う。tone 番号の専用トークンは追加せず、文言・URL・DOM 構造は変更しない** | §4.1、§8.4、§11.4.2 |
...
| 地図出典 | 既存の「地理院タイル」リンクの文字色に、既存 MD3 `--md-sys-color-inverse-primary` を使う。リンクの文言・遷移先・配置は維持 |
...
- ナウキャスト側の最新追従・手動保持・再生タイマー・再生中の一覧固定（F7 #50）。キキクルは本書の責務として常に最新 1 コマを表示し、選択可能な追従／保持モードや再生機能は設けない。
...
6. 種別切替時の選択時刻維持は、後続の「最新 1 コマのみ」確定により撤回し、常に切替先の最新コマを表示する（→ §5.3、§18）。
7. 表示窓は当初の過去 3 時間案を撤回し、最新 1 コマのみとする（→ §6.4、§18）。
...
9. 地図 UI はズーム 8 以下へ操作できないものとし、最小表示ズームを 9 とする。表示ズーム 9 では z10 気象タイルを縮小配置して、ナウキャスト・キキクルとも表示する（→ §7.2、§19）。

## 3. 実測（2026-09-15 実施）

本節は本書作成時に実データへアクセスして得た値を中心に記録する。画面寸法から導くタイル枚数など、実測ではない値は「概算」「算出」と明記して区別する。
...
**結論 1（決定的）**: **HTTP 200 が返ることは「そのズームに対応している」ことを意味しない。** 未対応ズームでも、上流は同一バイト列の 256×256 全透明 PNG を 200 で返す。`docs/data-acquisition-report.md` の「ズーム 10 以外は未検証」は、本実測により「z9・z11・z12 は、z10 に着色がある地点でも空タイルしか返さない」まで踏み込める。z8 と z10 では着色を確認しており、取得方法レポートの「偶数ズーム利用」の記述と整合する。
...
表示ズーム 9 では、z10 タイル 4 枚を各 128 CSS px 相当に縮小配置する。これは現行の `resolveTileZoomPolicy` が返す `nativeZoom=10` / `minDisplayZoom=9` と、`WeatherTileOverlay` の `minNativeZoom=maxNativeZoom=10` をそのまま活用する。`cmk-gsx-mockup` で直接確認できるのは**表示ズーム 10 超での拡大表示**である。同モックアップは表示ズーム 10 未満では表示ズームと同じ z のタイルを要求するため、**z10 タイルの underzoom（縮小表示）の実績とは扱わない。**
...
| `apps/web/src/map/MapViewport.tsx` | 変更（F3） | 地図インスタンスと背景地図レイヤーの `minZoom`、imperative `zoomOut()` の下限を 9 に統一する。初期ズーム 11、最大ズーム 18、会場復帰時のズーム 11 は維持する |
| `apps/web/src/map/MapZoomControls.tsx` | 変更（F3） | 既定の `minZoom` を 9 に合わせ、現在ズーム 9 で縮小ボタンを disabled にする |
...
| `apps/web/src/map/map.css` | 変更（追加改訂） | `.kikikuru-status-card .timeline-selected-time` をナウキャストと同じ 20px、768px 以下で 18px にする。`.attribution-link` の通常・visited・hover・focus-visible 時の文字色を既存の `--md-sys-color-inverse-primary` に統一し、既存の下線とフォーカス表示を維持する |
...
**出典リンクの追加改訂に限った許可変更ファイル**は、`apps/web/src/map/map.css` と直接対応する `apps/web/tests/mapControls.test.ts` の 2 本だけとする。`--md-sys-color-inverse-primary` は既にテーマから生成されるため、`applyTheme.ts` を含むテーマ実装は変更しない。`MapAttribution.tsx` も既存のクラスで CSS を適用できるため変更しない。リンクの文言・URL・`target` / `rel`・DOM 構造・配置、およびテーマシードは変更しない。
...
1. レイヤー切替処理では地図中心・ズームに**触らない**。`map.setView` / `flyTo` / `fitBounds` / `panTo` を呼ばない。`MapViewport` の `placementRef`（`'initial' | 'manual' | 'returning'`）も変更しない。地図 UI 全体の最小ズーム設定は §7.2 に従って別途 9 へ変更する。
...
**種別切替での即時再取得はしない。** キキクル API は 1 応答で 3 レイヤーすべてを返す（§6.1）ため、種別切替で再取得しても新しい情報は得られず API 呼び出しが増えるだけである。**`resetKey` にキキクル種別を含めない**（含めるのは `terminalId`、`controlStatus`、メインレイヤー識別子）。これは F2 設計書 §8.4 の確定済み共通契約と一致する。
...
- 地図 UI の最小ズームが 9 であり、ズーム 9 でも危険度分布を表示するため、縮尺不足を理由とする非表示・案内文言は設けない。
...
**結論【確定】**: 地図全体の表示・操作範囲を **9〜18** とする。初期ズーム 11 と会場復帰時のズーム 11 は維持する。表示ズーム 9 でも、`resolveTileZoomPolicy([10])` の `minDisplayZoom=9` と `WeatherTileOverlay` の `minNativeZoom=maxNativeZoom=10` により、z10 気象タイルを縮小配置してナウキャスト・キキクルを表示する。ズーム 8 以下で気象レイヤーだけを非表示にする従来案は撤回し、`MapViewport` の Leaflet `minZoom`、背景地図レイヤーの `minZoom`、`MapZoomControls` の既定 `minZoom` を 9 に揃えて、地図 UI 自体をズーム 8 以下へ到達不能にする。

このユーザー確定事項は、F1 設計の「背景地図 5〜18」と F2 設計の「8 以下では気象レイヤーだけを外す」を、アプリの操作可能範囲について置き換える。国土地理院タイル自体の提供範囲 5〜18 という外部仕様は変わらないが、本画面から利用できる範囲は 9〜18 とする。F2 の `resolveTileZoomPolicy` と `WeatherTileOverlay` は変更せず、`minDisplayZoom=9` を防御的な表示条件としてそのまま利用する。

ズーム 9 のフル HD 相当では概算約 162 枚／コマを要求する。F2 設計の約 144 枚とは想定した主領域寸法・端数の取り方が異なる概算であり、いずれも実測値ではない。本書では約 162 枚を検収時の比較基準とし、実要求数を記録する。キキクルは最新 1 コマのみで先読み・再生を行わないため許容可能と見込むが、応答時間と表示完了は検収で実測する。ズーム 8 の概算約 600 枚／コマは UI から到達不能にして回避する。縮小ボタンだけでなく、ホイール、トラックパッドのピンチ、タッチピンチ、キーボード、Leaflet API による通常操作でも 8 以下にならないことを Leaflet の `minZoom=9` で一元的に保証する。レイヤー切替時は従来どおり中心と現在ズームを保持し、ズーム 9 から勝手に 10 へ補正しない。
...
種別切替（`selectedLayerId` の変化）時は、`urlTemplate` が変わるだけで同じ差替え機構が働く。§5.3 のとおり切替先の最新 1 コマを再解決する。3 種の最新 `validTime` が一致する通常応答では表示日時を維持したまま画像だけが入れ替わるが、時刻一致を前提条件にはせず、切替先の最新時刻が異なれば簡易カードもその時刻へ更新する。
...
| --- | --- | --- |
| `KikikuruApiFrame.validTime` | **危険度判定の基準時刻**（§3.1 実測により `baseTime` と常に等しい） | `KikikuruStatusCard` の最新時刻として表示する唯一の時刻 |
...
- `applyMd3Theme()` は scheme の全色を `--md-sys-color-*` として書き出しており、`--md-sys-color-inverse-primary` は既に利用できる。追加トークンやテーマ生成処理は不要である。
- `docs/rules/06-ui-md3-protocol.md` は画面 CSS での HEX 直書きを禁止し、`apps/web/src/theme/` が生成する `--md-sys-color-*` の利用を必須としている。

#### 採用する設計

1. `.attribution-link` の通常、`:visited`、`:hover`、`:focus-visible` の**文字色**はすべて `var(--md-sys-color-inverse-primary)` とする。visited 時にブラウザー既定の紫へ変わらず、hover / focus 時にも指定色が別色へ退行しない。
2. リンクは通常時から下線を維持し、色だけにリンク識別を依存させない。`:focus-visible` の既存 2px outline はキーボードフォーカスを明確にするため維持し、outline 色には既存の `--md-sys-color-primary` を使う。リンク文字とフォーカス枠で既存 MD3 トークンの役割を分ける。
3. `MapAttribution.tsx` はリンクの意味・遷移・アクセシビリティを、`map.css` は通常／visited／hover／focus-visible の見た目を担う。既存クラスで完結するため TSX は変更しない。
4. tone 番号を名前に含む専用トークン、HEX リテラル、`applyMd3Theme()` の追加処理は設けない。

#### コントラストの扱い

`--md-sys-color-inverse-primary` はユーザー確定値である一方、リンク直下は単色面ではなく、`grayscale(1) brightness(0.66)` 適用後の地理院タイルで画素ごとに明度が変わる。したがって MD3 トークンであることだけを根拠に常時適合すると推論せず、1920×1080 相当と 1024×768 相当、会場 2 地点でリンク文字と直下背景の代表画素を実測する。11px の通常文字としてコントラスト比 **4.5:1 以上**を合格基準とする。下線はリンク識別の非色手掛かりであり、文字コントラスト不足の代替にはしない。
...
| `apps/web/src/map/MapViewport.tsx` の既存改変 | `MapViewportHandle` の Leaflet インスタンス取得口。F3 はこの取得口を変更しない。ただし §7.2 のユーザー確定事項に従い、地図インスタンス・背景レイヤー・imperative `zoomOut()` の下限を 5 から 9 へ変更する |

**F3 製造担当への指示**: 着手時に上記が `main` に存在することを確認する。存在しない、または仕様が上表と食い違う場合、**自己判断で F3 側に別実装を起こさず、統括担当へ差し戻す。** `apps/web/src/map/WeatherMapView.tsx` と `apps/web/src/App.tsx` は F2 も変更するため、F2 のマージ後の最新 `main` を基点にすること。`resolveTileZoomPolicy` と `WeatherTileOverlay` の native zoom／表示下限ロジックは変更せず、共通地図 UI の到達可能範囲だけを `MapViewport` / `MapZoomControls` で 9〜18 に制限する。

F2 設計 §6.1・§11.3 に残る「表示ズーム 8 以下では気象レイヤーだけを外す」という検証項目は、本書 §7.2・§19 の後発ユーザー判断により撤回された。ズーム 8 以下を作ってレイヤー非表示を試験するのではなく、ズーム 9 から先へ縮小できず、表示画像とタイル要求が変化しないことを §11.5 で検証する。
...
- **表示窓**: 雨雲の時間軸に対し、キキクルは最新 1 コマのみ（§6.4）である。共通フック側で窓や最新 1 コマへの絞り込みを適用しない。
- **`member` の再解決**: キキクル固有（§6.3）。共通側へ持ち込まない。
- **`opacity`**: 雨雲とキキクルで別定数とする（§7.3）。
- **`resetKey` の構成**: F2 設計書 §8.4 と同じく、キキクル API は 1 応答で 3 レイヤーを返すため、**F3 は `resetKey` に種別を含めない。** 種別切替は `urlTemplate` の差し替えだけで完結する。
...
- [ ] `getComputedStyle()` で、`.attribution-link` の通常・visited・hover・focus-visible の文字色がすべて既存の `--md-sys-color-inverse-primary` の解決値と一致する。ブラウザー既定の visited 色へ変わらない。`mapControls.test.ts` も各状態の CSS 境界を直接検証する。
- [ ] 通常時から下線が表示され、hover でも下線が消えない。キーボード操作の `:focus-visible` では `--md-sys-color-primary` を使う既存の 2px outline が視認でき、文字色は `--md-sys-color-inverse-primary` のままである。
- [ ] tone 番号の専用トークン、HEX リテラル、`applyMd3Theme()` の追加処理がなく、`applyTheme.ts` とテーマ関連テストにこの追加改訂の差分がない。
...
- [ ] `git diff --name-only` で、この追加改訂のコード差分が §4.1 の許可 2 ファイルに収まっている。テーマ実装・テーマシード、他の地図コントロール、気象タイル色、カード構造に差分がない。

### 11.5 ズーム・タイル座標・凡例位置（AD-H058）

- [ ] キキクル表示中に DevTools の Network タブでタイル要求を確認し、**表示ズームが 9、11、13 のいずれでも、要求 URL の `z` が常に `10`** である。ナウキャストについても表示ズーム 9 で要求 URL の `z` が `10` である。
- [ ] `east` 端末の初期表示（ズーム 11、会場中心）で、要求されたタイル座標の集合に **`10/909/403`** が含まれる。`trc` 端末でも同じく `10/909/403` が含まれる。
- [ ] 地図の初期ズームが 11、最大ズームが 18 のままであり、「会場の初期位置に戻る」で中心を会場へ戻した際もズーム 11 になる。
- [ ] 表示ズーム 9 でキキクルとナウキャストの気象レイヤーがともに表示される。z10 タイルが 128 CSS px 相当に縮小配置され、レイヤー切替の前後で地図中心とズーム 9 が維持される（10 へ自動補正されない）。
- [ ] ズーム 9 では縮小ボタンが disabled になり、クリックしてもズームが変わらない。拡大ボタンは enabled のままである。
- [ ] ズーム 9 でホイール、トラックパッドのピンチ、タッチピンチ、キーボードの縮小操作をそれぞれ行っても `map.getZoom()` が 9 未満にならない。Leaflet の通常 API でも `map.zoomOut()` と `map.setZoom(8)` を試し、9 にクランプされる回帰テストが通過する。
- [ ] ズーム 9 で表示中の中心座標・気象画像・Network のタイル要求 URL 集合を記録してから縮小ボタンを押す。中心座標と表示画像が変化せず、`zoomend`、新しい気象タイル要求、オーバーレイ差し替えが発生しないことを確認する。これにより「9 からの縮小操作で表示とタイル要求が変化しない」を固定する。
- [ ] `MapViewport` の地図インスタンス・背景地図レイヤー・imperative `zoomOut()` と、`MapZoomControls` の既定最小ズームがすべて 9 に揃っている。対象コードと更新したズーム境界テストに旧既定値 5 が残っていないことを確認する。
- [ ] 1920×1080 相当で表示ズーム 9 のキキクル最新 1 コマを表示し、Network タブからタイル要求数と、要求開始から全タイルの完了または `onSwapSettled` までの時間を記録する。概算約 162 枚との差を説明し、タイムアウト・操作不能・継続的な再要求がないことを確認する。未実測のまま性能合格としない。
...
- [ ] キキクル表示中はスライダー自体が DOM に存在せず、簡易カードのアクセシブルな名前・時刻表現にも「実況」「予報」が含まれない。
...
- [ ] キキクル表示中は再生・前へ・次へ・最新へボタンとスライダーが DOM に存在せず、キキクルの再生機能を実装済みと報告しない。
- [ ] 60 秒ポーリングで新しい索引が届いたときは、最新 1 コマの `validTime` とタイル URL が新しい最新コマへ更新される。これはユーザー選択可能な「最新追従」状態ではなく、§6.4 の最新 1 コマ抽出そのものである。
...
初版の未決事項は 2026-09-15 のヒアリングと資料支給で確定した（§0）。**階級色（初版 §12-1）と `#F2E700` の階級割当（初版 §12-2）は §7.4.2 で確定済みであり、未決から外した。** `resetKey` は F2 設計 §8.4 と一致し、キキクルの時刻は最新 1 コマのみという後続判断で確定済みである。地図の最小表示ズーム 9 も §7.2・§19 のユーザー判断で確定しており、未決事項ではない。残る未決は次の 1 点だけである。
...
| 項目 | 本 Issue での結論 |
| --- | --- |
| **AD-H057** タイルの選択・部分欠け | **キキクルでは同一 `validTime` の複数候補は発生しない。** 上流 `targetTimes.json` は 1 時刻につき 1 `member` しか返さず（37 件で `validtime` 重複ゼロ、実測）、`kikikuruService` は取得成功のたびにスナップショットの `frames` を全面置換するため、保存索引にも重複が生じない。ただし `member` は時間とともに `immed0`→`immed1`→`immed2`→`none` とドリフトするため、フロントは `validTime` を id とし、タイル要求の直前に最新索引から `member` を引き直す（§6.3）。部分欠けは `errorTileUrl` で埋めず透明のまま残し、件数のみ `status` slot へ出す。欠けの分類・前回画像の時刻表示は F8 (#51) に残す。キキクルは最新 1 コマのみで操作部を持たないため、再生中一覧固定や再生タイマーの受け口は設けない。 |
| **AD-H058** PNG 完全 decode・位置/凡例 | **完全 decode を実施し成功した。** `rain_mesh/10/892/408` は 256×256 / bitDepth 8 / colorType 6 / 非インターレース、zlib 展開後 262400 バイトを全行フィルタ解除まで通した。**位置**は標準 XYZ / EPSG:3857 であり、会場の z10 タイルは両会場とも `10/909/403`（取得方法レポートの実検証座標と一致）。**zoom** は上流の z8 と z10 で着色を確認した一方、z9・z11・z12 は z10 に着色がある地点でも同一バイト列の全透明 PNG を 200 で返す（＝HTTP 200 は対応の証拠にならない）。フロント API が許可するのは `[10]` だけなので `minNativeZoom=maxNativeZoom=10` を維持し、表示ズーム 9 では z10 タイルを縮小配置する。地図 UI の操作範囲は 9〜18 とし、概算約 600 枚となるズーム 8 以下には到達させない。**凡例**の位置は F5 実装済みの左上を維持する。**階級区分・名称は公式マニュアル（別表 3、令和 4 年 6 月以降）から確定させた**（§7.4.1）。**階級色は公式配色（HP 配色設定指針・警戒レベル対応ページ）から 5 段階すべてを確定させ、データ色トークン `--wx-data-kikikuru-*` として定義した**（§7.4.2）。実測した着色 `#F2E700` は公式値 rgb(242,231,0)「注意」と**完全一致**し、透明画素 `(255,255,255,0)` も「今後の情報に留意」の白と一致した。着色部が α=255 の完全不透明であることを確認し、重畳は `opacity: 0.75` とする。**透過度の実データでの視認性と表示ズーム 9 の約 162 枚／コマの応答性能は実挙動未確認**であり §12-1、§15 に残す。 |
...
| 後続 | 引き継ぐもの | 先取りしないもの |
| --- | --- | --- |
| F2 #45（**先行**） | §10 の共通モジュール仕様（`TILE_CATALOG_POLL_INTERVAL_MS = 60_000`、`resolveTileZoomPolicy` による `nativeZoom=10` / `minDisplayZoom=9`）を流用する | 最新 1 コマへの絞り込み、キキクル固有の `member` 再解決。共通地図 UI の最小ズーム 9 への変更は F3 で行う |
| F7 #50 | ナウキャスト再生中にキキクルへ切り替えた場合は再生を停止する接点。キキクル側はポーリングごとに最新 1 コマへ更新する | キキクルの再生タイマー、手動保持、過去コマ選択は不要。ナウキャスト側の最新追従・手動保持は F7 が所有する |
...
5. 表示ズーム 9 における概算 162 枚／コマのタイル同時要求数と実測応答時間。キキクルは最新 1 コマのみであることを前提に、§11.5 の手順で検収する。
...
| `selectedFrameId` による過去時刻の保持 | 表示ズーム 9 での z10 タイル縮小表示 |
...
- **地理院タイル出典リンクに `--md-sys-color-inverse-primary` を使う**: 【確定】。tone 番号の専用トークンとテーマ生成処理は追加せず、HEX 直書き、文言・URL・構造・配置の変更は行わない。
...
## 19. 地図 UI の最小表示ズーム変更（2026-09-22）

ユーザー判断により、地図 UI の最小表示ズームを 9 とし、ズーム 8 以下へは操作できない仕様へ変更した。従来の「背景地図は 5〜18 のまま、ズーム 8 以下では気象レイヤーだけを非表示にする」案は撤回する。

### 19.1 確定した挙動

- 地図の操作可能範囲は **9〜18**。初期表示と会場復帰は従来どおりズーム 11 とする。
- `MapViewport` の Leaflet 地図インスタンス、地理院淡色地図レイヤー、imperative `zoomOut()` の下限を 9 に揃える。
- `MapZoomControls` の既定 `minZoom` を 9 とし、ズーム 9 では縮小ボタンを disabled にする。
- ホイール、トラックパッド／タッチのピンチ、キーボード、Leaflet の通常ズーム API のいずれでも 9 未満へ到達させない。個別入力を手作業で抑止するのではなく、Leaflet の `minZoom=9` を最終境界とする。
- 表示ズーム 9 では `allowedZooms=[10]` から決めた z10 タイルを 128 CSS px 相当に縮小配置し、ナウキャストとキキクルをともに表示する。`resolveTileZoomPolicy` の `nativeZoom=10` / `minDisplayZoom=9` と `WeatherTileOverlay` の `minNativeZoom=maxNativeZoom=10` は変更しない。
- ズーム 9 からさらに縮小操作しても、地図中心、表示ズーム、気象画像、タイル要求 URL 集合は変化しない。

### 19.2 性能判断と実測境界

フル HD 相当の表示ズーム 9 は約 162 枚／コマ、ズーム 8 は約 600 枚／コマの概算である。キキクルは最新 1 コマのみなのでズーム 9 を許容候補とするが、約 162 枚という値と表示完了時間はいずれも実挙動未確認であり、§11.5 に従って検収で実測する。ズーム 8 は性能試験で気象レイヤーだけを外す対象ではなく、UI から到達不能にして回避する。

`cmk-gsx-mockup` は表示ズーム 10 超での z10 タイル拡大表示だけを直接確認できる。表示ズーム 10 未満では表示ズームと同じ z のタイルを要求しているため、z10 underzoom の直接実績として引用しない。本件の z10→表示ズーム 9 は、現行共通部品の設定を用いたうえで本アプリの検収により確認する。
