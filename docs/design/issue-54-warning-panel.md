...
- 実電文(東京対象の VPWW54〜61/VPWS50)はクラウド環境に無く**未確認**。特に「新規発表時の `LastKind` の有無・値」「強化時の `Status` の値」は実電文で確認していない(§9)。
...
- **表にないコード**(未定義・予約コード)は描画しない。【設計案・要ヒアリング Q1】黙って消すと発表中情報を見落とす恐れがあるため、表示方法は統括確認とする。確認までは `unknownCodes` を返すだけで描画しない実装とし、低い段階へ自動割り当てしない(§7.4)。
...
- 時刻の書式は G1 共通の `formatPanelTime`(当日 `HH:mm発表`)に従う。§5.7/Issue の「○月○日○時○分 発表」とは異なる。【要ヒアリング Q2】
...
- 新規発表時に電文が `LastKind` を持つか(持つ場合に `'00'` 等になるか)、強化時の `Status` が `発表` か は実電文未確認。`'00'` の扱いは推測で `new` にしない。製造前にローカル環境の実電文で確認が必要(§9、要ヒアリング Q3)。
...
- 【未確認】新規発表時の `LastKind` 有無・値、強化時の `Status` 値(§4.3)。東京対象の実電文で未確認。
- 【未確認】VPWS50 定期集約の反映で、個別電文による `発表`/`XからY` が `継続` に置き換わり強調が短時間で消える可能性(§2.1-1)。
- 【未決】表外コードの表示(Q1)、見出し時刻の書式(Q2)、`'00'` 等の新規判定(Q3)。
