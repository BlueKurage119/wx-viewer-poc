...
- 新規発表時に電文が `LastKind` を持つか(持つ場合に `'00'` 等になるか)、強化時の `Status` が `発表` か は実電文未確認。`'00'` の扱いは推測で `new` にしない。ユーザーがローカルの実電文で確認中であり、**結果待ちの未決事項**とする(Q3)。「新規」の判定条件は確認結果を受けて本書を改訂する。改訂までは上表のとおり実装する。
...
- 【未決・確認待ち】`LastKind` の有無・値と強化時の `Status`(Q3)。ユーザーがローカルの実電文で確認中。結果を受けて §4.3 の「新規」判定を改訂する。
