...
状態: 承認済み。ユーザーの「はい、その通り実施してください」により、ツールバー改訂の製造とヘッダー比較検証用の変更を承認。ヘッダーの恒久解決は実機確認待ち。
...
初回製造のセーフエリア対応は維持する。ユーザー実機では重なりが残っているため、以下のmeta追加を比較検証用の候補として製造することが承認された。実機確認が成功するまでは恒久解決として扱わない。

- `apps/web/index.html`に`<meta name="apple-mobile-web-app-capable" content="yes" />`と`<meta name="apple-mobile-web-app-status-bar-style" content="black" />`を追加する。非透過のステータスバー指定を試すもので、`black-translucent`にはしない。
- 既存viewportとCSSセーフエリアを維持し、OS用の固定px余白、manifest、Service Worker等のPWA機能は追加しない。
- 比較では同じ端末URLをiPadOS 27.0のSafariで読み込み、新しくホーム画面に追加したアイコンから起動する。通常Safari、新規アイコン、既存アイコン、旧画像を区別して記録し、上端のボカシとヘッダーの位置関係を比較する。既存アイコンの設定が更新されない可能性と、新規アイコンでも改善しない場合を区別する。既存アイコンの削除は比較の前提にしない。
...
- [ ] AC6: 2.3の比較手順で通常Safari・既存アイコン・新規アイコンの違いと旧画像からの変化を記録する。同じiPadを新規アイコンでホーム画面から起動し縦横へ回転する。ヘッダーのタイトル・時刻・端末名・受信異常表示がOSステータスバーの下に収まり、アラーム停止操作が利用できること。下端の通知操作が隠れないこと。通常Safariでも余白の二重適用がないこと。
...
- **ユーザー実機報告**: 初回製造後、ヘッダーのボカシとの重なりは未解決、それ以外はすべて異常なし。各ACのチェックは、個別の実施手順・結果が確認できるまで自動的に完了扱いにしない。
- **実挙動未確認**: 対象iPadのOSは27.0と確認済み。ホーム起動時のセーフエリア値、ヘッダー問題の原因、左右固定・中央スクロールへの改訂後の縦横表示と操作は未確認。初回の再読み込み問題についても原因は特定していない。
...
- ヘッダーは非透過meta追加の比較検証用変更まで承認済み。恒久解決は実機結果を受けて判断する。画像ではOS文字列そのものより、上端のボカシがヘッダーへ及んでいる。セーフエリア不足とは未確定のため、実測前の固定px追加や`black-translucent`導入を確定しない。AC6は未解決として維持し、原因・方式・再確認結果を後で記録する。
- 統括調査の切り分け候補: [Appleの旧MetaTags資料](https://developer.apple.com/library/archive/documentation/AppleApplications/Reference/SafariHTMLRef/Articles/MetaTags.html)は`apple-mobile-web-app-capable=yes`併用の`apple-mobile-web-app-status-bar-style=black`または`default`で本文がバーの下に配置されると説明する。初回製造の`index.html`には両metaがなく、iPadOS 27.0での効果は未実測。非透過の`black`設定を比較検証用に追加し、初回設定と実機で比較する。恒久採用の判断はまだ行わない。統括が参照したWebKit #316008にはホーム追加時のmetaが後から反映されない別の高さ問題の報告があるが、本件と同一原因とは断定しない。
