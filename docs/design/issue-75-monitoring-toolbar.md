...
1. 「取得開始」「取得停止」「強制更新」は単一選択。別ボタンで切替、同じボタン再押下で解除。選択・解除ではAPIを呼ばず、監視データも変更しない。
...
- 戻る2個は40×40px。既存の `.monitoring-toolbar-group md-gb-button { inline-size:120px }` を専用クラスで限定的に上書きし、内容padding/min-widthで40pxを超えないか実ブラウザで確認する。正方形の外寸とMaterialのsquare形状は別々に確認する。
...
- 選択はaria-pressed、MD3トークンによる枠/背景、操作行の文字を組み合わせる。disabled、送信/照会中の文字を維持し、色だけで状態を表さない。

### 7.4 キーボードと読み上げ

- 戻るボタンはaria-label/titleをそれぞれ「最初のメニューへ戻る」「一つ前のメニューへ戻る」とする。アイコンspanはaria-hidden。文字付きボタンでもアイコン名を読み上げない。
- 実装済みGbButtonのホストにaria属性を付けただけで完了とせず、内部buttonのアクセシビリティツリーとdisabled時のキー操作をブラウザで検証する。必要な属性転送はラッパー内に限定する。
- Tab/Shift+TabとEnter/Spaceで利用できる。矢印キー移動を実装しない状態で `role="toolbar"` を付けず、名前付きgroupでまとめる。フォーカス輪郭はMD3のoutline/primary等を使用する。
- 操作行は既存 `role="status" aria-live="polite"` を利用し、同じ202の反復で同一文言を書き直して読み上げを連打しない。ボタンに別のlive領域を追加しない。
...
- [ ] **AC19 配置・寸法**：フォントとMaterial登録完了後、内容幅1448px/1447px/600pxで測定する。11個のボタン、既存文字ボタン120×40px、戻る40×40px、名前120pxを確認。境界前後でグループ単位折返し、最後のクリア/送信グループ右寄せ、横溢れ・ボタン縮小なし。本文・通知の重なりを確認する。
- [ ] **AC20 名前と状態の安定**：テスト専用子メニューで短い/長いタイトルを切り替える。タイトル領域幅と後続先頭ボタンのx座標が変わらない。ルートでは文字だけ空になる。未選択/選択/送信中/disabledの高さが変わらない。
- [ ] **AC21 アクセシビリティ**：Tab/Enter/Spaceで一連の操作を行う。戻るのaria-label/title、アイコン非読み上げ、aria-pressed、内部buttonのdisabled、フォーカス可視、操作行の状態通知をブラウザで確認。問いかけの送信と取得操作の送信を識別できる。
...
- Material Labsの40pxアイコン専用外寸、アイコン/ラベルの収まり、実際の形状遷移、内部buttonへのaria属性伝播、ダイアログのフォーカス制御は**実挙動未確認**。
...
- 終了時の差分は本書の新規追加のみ。`git diff --no-index --check /dev/null docs/design/issue-75-monitoring-toolbar.md` で空白エラーなしを確認した。依存関係（node_modules）が未導入のためPrettierは実行していない。コード変更・コミットを伴わない設計段階のためlint/typecheck/テスト/buildは実行していない。
