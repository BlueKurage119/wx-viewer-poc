...
3. 選択により送信が可能になったときは、送信ボタンだけを通常配色とinverse配色の段階的な完全切替で強調する。POST待機と結果照会中は取得操作3個・クリア・送信を無効にし、送信可能アニメーションも同期的に停止する。内部でも同期的な実行中ガードを置き、同じ描画フレームの二重押下を防ぐ。
...
- 配色は既存 `--md-sys-color-*` のみを、Material GB buttonが公開するCSS part/custom properties経由で使用する。固定HEX/RGB/HSLや新規色トークンは追加しない。通常の文字付き操作ボタンは背景 `var(--md-sys-color-primary-container)`、文字と併記アイコン `var(--md-sys-color-on-primary-container)` とする。ここで通常とは、選択中・送信可能・disabledの専用状態に該当しない取得操作、履歴・診断、クリア、送信をいう。disabledはMaterial標準と同じ相対色構文を明示し、背景を `hsl(from var(--md-sys-color-on-surface) h s l / 10%)`、アイコンと文字を `hsl(from var(--md-sys-color-on-surface) h s l / 38%)` とする。このトークン由来の相対色構文は固定色直書きに含めない。文字付きボタンでは公開 `--container-color`、`--icon-color`、`--label-text-color` と前面container/contentを同期し、Icon Buttonでは公開 `--container-color`、`--icon-color` を使う。
...
- 送信可能（未送信の取得操作が選択済み、かつbusyでない）では、送信ボタンの実表示となる公開container slotの背景と、文字・矢印アイコンへ同期した `monitoring-send-ready` アニメーションを適用する。1周期は正確に2秒、`steps(1, end)`相当の段階切替、無限反復とし、前半を通常配色、後半を背景 `var(--md-sys-color-inverse-surface)`・文字と矢印アイコン `var(--md-sys-color-inverse-primary)` とする。切替途中の色補間は行わず、常にいずれか一方の完成した配色を表示する。opacity、visibility、display、transform、filter、outline、寸法、padding、角丸をkeyframesへ含めず、ボタン自体を表示/非表示にする明滅は行わない。
...
- [ ] **AC21 アクセシビリティ・配色・送信可能表示**：Tab/Enter/Spaceで3取得操作をそれぞれ選択・解除し、実ブラウザAX上の名前が「取得開始」↔「取得開始、選択中」等へ完全一致で切り替わることを確認する。通常button・公開aria-label・状態クラスを使い、toggle/selected/aria-pressed・内部ARIA注入・形状遷移を使わない。機械テストで通常、選択中、送信可能、disabled/busy/離脱、reduced-motionのクラス・CSSを検査し、選択専用outlineがなく、通常はprimary-container/on-primary-container、選択中はtertiary-container/on-tertiary-container、disabledはon-surface由来の背景10%・アイコン/文字38%であることを確認する。disabledの文字付きボタンとIcon Buttonについて、公開カスタムプロパティと実表示層の解決済み色が一致し、操作不能・animationなしを実ブラウザで確認する。送信可能時は実表示containerと文字・矢印に同期したanimationが `2s steps(1, end) infinite`相当で、前半が通常配色、後半がinverse-surface/inverse-primaryとなり、色以外のkeyframe propertyがないことを確認する。実ブラウザでは周期の前半・後半で解決済み背景・文字・矢印色を測定し、中間補間色が表示されないこと、送信開始・disabled・選択解除・離脱ではanimationが0件になることを確認する。reduced-motionではanimation 0件かつinverse配色固定とする。light/dark双方の通常・inverse配色を相対輝度から再計算し4.5:1以上とする。状態クラスと接頭辞なしの操作行が追従し、既存フォーカス可視性、`role="status"`、戻るaria-label/title、アイコン非読み上げ、問いかけ送信との識別も確認する。
...
【確定】送信可能時は送信ボタンだけを通常配色と`inverse-surface`/`inverse-primary`の間で段階的に完全切替する。仕様値は2秒/周期、無限反復とし、前景と背景を同期して常にどちらかの完成した配色を表示する。連続補間、ボタン自体の表示/非表示、色以外の変形は行わない。disabled、送信開始、選択解除、画面離脱で停止する。`prefers-reduced-motion: reduce`ではアニメーションせずinverse配色へ固定する。これは製造中の実測で、明暗関係が反転する2組を連続補間すると途中コントラストが最低約1.317:1となり、4.5:1を維持できないと判明したため、ユーザーが完全な段階切替を許容し統括がコントラスト優先を選択した追加判断である。現行テーマ生成値ではlight/dark双方の通常・inverse各組が4.5:1以上であるが、製造後も解決済み色で再検証する（§7.3、AC20/AC21）。
...
【確定】追加監修により、非活性ボタンはMaterial GB button既定と同じon-surface由来の相対色へ統一する。背景は10%、アイコンと文字は38%のalphaとし、文字付きボタンとIcon Buttonの公開カスタムプロパティおよび前面表示層へ同じ値を適用する。固定色は追加せず、相対色構文をテーマ追従のための許可された指定として扱う（§7.3、AC21）。
...
- §10-Dの初回改訂ではコード・テスト・ブラウザを変更せず、導入済みMaterial Webの型定義/CSSと現行テーマ生成だけを静的確認した。製造中の実測で連続補間とコントラスト基準が両立しないと判明したため、上記追加判断により2秒の段階切替へ改訂した。`GbIconButton`の40×40px完全表示、選択配色、段階切替、停止条件、reduced-motion固定色、操作行文言は独立検収担当がAC17/AC19〜AC23で再検証する。
