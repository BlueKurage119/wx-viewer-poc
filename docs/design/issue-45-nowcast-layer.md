...
#### 9.3.1 なぜ逐次読込では 1 秒を満たせないか

- API は全応答に `Cache-Control: no-store` を返すため、ブラウザキャッシュが効かない。同じ URL でも再要求すれば必ず再ダウンロードになる。
- 1 コマは初期ズーム 11 で 12 枚、ズーム 9 で約 144 枚のタイルからなる。ブラウザの同時接続数と API 側のキャッシュ未命中が重なり、1 コマの `load` 完了までに実測 2.7〜4.2 秒かかる。
- **先読みの深さ 1（次の 1 コマだけを背景で読む）では足りない。** 表示時間 1 秒の間に稼げる先行は 1 秒分であり、1 コマ 2.7〜4.2 秒の読込時間を吸収できない。1 秒間隔を維持するには、読込を**複数コマ並行**で進めるか、**一度読んだレイヤーを捨てずに保持**する必要がある。
...
2. **先読み（パイプライン）**: 1 周目は、現在のコマを表示したまま、後続 `PLAYBACK_PREFETCH_DEPTH` コマ分のレイヤーを `opacity: 0` で先行して `addLayer` し、並行して読み込ませる。`PLAYBACK_PREFETCH_DEPTH = 3` とする【設計案】。深さ 3 は、1 コマ 2.7〜4.2 秒 ÷ 目標間隔 1 秒 ≈ 3〜4 という比から定めた最小値である。
...
| 再生中 1 周目 | **最大 `1 + PLAYBACK_PREFETCH_DEPTH` = 4 枚** | active 1 + 先読み 3 |
| 再生中 2 周目以降 | **最大でコマ一覧の件数（表示窓いっぱいで 25 枚）** | 読込済みを保持 |
...
数値条件は周回によって分ける。**1 周目はネットワーク律速のため 1,000 ms を保証しない。2 周目以降は読込済みレイヤーの保持により `opacity` 入れ替えだけで切り替わるため、1,000 ms ± 300 ms を保証できる**（§9.3.2）。この区別は逃げではなく、`Cache-Control: no-store` のもとで達成可能な上限を示したものである。
...
- [ ] **【数値条件】1 周目**の各間隔を同様に記録し、**中央値が 2,000 ms 以下**であることを確認する。先読みなしの実測（2.7〜4.2 秒）より改善していることを、改訂前の測定値と並べて記録する。
- [ ] 再生中に地図へ同時に載るタイルレイヤー数が §9.3.3 の上限（1 周目は最大 4 枚、2 周目以降はコマ一覧の件数まで）に収まることを、`map` の内部レイヤー数または DOM 上の `.leaflet-layer` 数で確認する。
...
- [ ] `WeatherTileOverlay` に `id` が同一で `urlTemplate` だけが異なる `frame` を渡すと差し替えが走ることを単体テストで確認する（§8.3）。`grep -n "frame.id" apps/web/src/map/tiles/WeatherTileOverlay.tsx` で、差し替え可否の判定に `frame.id` 単独の比較が残っていないことを確認する。
...
4. **先読み＋リテンション方式での実測間隔**。1 周目・2 周目の間隔、保持枚数、メモリ使用量はいずれも改訂後の実測が未実施であり、§11.4 で確認する。改訂前の逐次読込方式では 1 コマ 2.7〜4.2 秒（検収実測）であった。
5. **新規 REST をフロントから呼んだときの挙動全般**。#39 設計も新規 REST を実挙動未確認としている。
6. **表示ズーム 9（1 コマ約 144 枚）での要求枚数と再生の実負荷**。枚数は机上算出であり、実ブラウザーでの実測、保持時のメモリ（約 3 MB 見積り）、2 周目で 1,000 ms を満たせるかは §11.3・§11.4 で確認する。
...
6. 先読み深さ `PLAYBACK_PREFETCH_DEPTH = 3`（§9.3.2）。1 コマの読込時間 ÷ 目標間隔の比から導いた最小値であり、§11.4 の実測で不足なら増やす。
7. キキクル（F3）でも先読み・リテンションを使うか。キキクルは未来時刻を持たず再生の重要度が低いため、本書では既定オフとし採否を F3 側の判断に残す。
