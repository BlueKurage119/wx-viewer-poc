...
改訂日: 2026-09-09（公式提供サンプルを用いた初回検収結果を反映）
...
この Issue で得るのは「1 電文から読み取れた江東区向け Kind の正規化結果」である。`Control/Status` の通常・訓練・試験を保持し、`Body/Warning` の対象種別を選んだうえで、`Item/Area/Code=1310800` に属するすべての `Kind` について、`Name`、`Code`、`Status`、`LastKind`、出現順を保った全 `Property`、`Addition`、`Kind/DateTime` を失わずに取得する。`Status` だけの公式な「発表警報・注意報はなし」も、正常な Kind として明示表現する。
...
初回検収で公式提供サンプルと実装を再照合した結果、既存実装の `parseKind` は `Property` を最大 1 件としていたが、VPWW61 には同一 Kind 内に 2 件ある。また `Name`・`Code` を全 Kind で必須にしていたが、VPWS50 には `Status` だけの正常 Kind がある。日時の既存実装は `new Date(raw)` の成否だけを見ており、日付だけの文字列も受け入れていた。これら 3 点は実データで通常到達する設計不一致として、本改訂で型、分岐、エラー分類、テスト、受け入れ条件を揃えて修正対象とする。
...
ただし、直下要素として `Status` だけを 1 件持ち、その値が完全一致で `発表警報・注意報はなし` の Kind は、公式提供サンプルの VPWS50 に存在する正常 Kind として受け入れる。この Kind は構造不正でも対象地域外でもなく、空の `kinds` 配列にも変換しない。後述の `kindType: 'no_warning'` と `sequence`、`status` だけを持つ 1 件の結果として、存在した Kind 自体を明示する。存在しない `Name`・`Code`・`LastKind`・`DateTime`・`Property`・`Addition` を `null` や空配列で補わない。`Status` がこの値以外なら通常 Kind として `Name` と `Code` を必須にし、Status だけの Kind を許容しない。この Status と他の Kind 項目が同居する未確認構造も、意味を推測せず `未対応構造` とする。
...
C1 が個別 XML を取得して telegram_reception に原文を保存
  └─ C2 processor（保存成功後に 1 回）
       ...
       │       正常な発表なし: kindType / sequence / Status のみ
...
10. `Kind/Status` が `発表警報・注意報はなし` だけを持つ公式正常 fixture は、`{ kindType: 'no_warning', sequence: 1, status: '発表警報・注意報はなし' }` と完全一致する正常結果となる。`Name`・`Code` 欠損を構造不正や対象地域外、空の `kinds` 配列にせず、存在しない項目も補わない。一方、同じ Status 以外で `Name`・`Code` を欠く Kind、およびこの Status と他の Kind 項目が同居する fixture は `未対応構造` となる。
...
今回追加する各回帰テストは、対応する旧挙動で先に red を確認する。具体的には、複数 Property fixture は「最大 1 件」の制限または先頭 1 件への縮退で失敗し、Status-only fixture は全 Kind への Name / Code 必須化で失敗し、日付のみ fixture は `new Date(raw)` だけの判定で誤って成功することを確認する。その前に、意味を変えない fixture の整形だけを変更してテストが成功し続ける対照実験を行う。次に、それぞれ Property の 2 件目を捨てる、no-warning 分岐を外す、日時の字句検証を外すミューテーションが該当テストにより KILLED となることを確認する。一時変更は完成コードへ残さない。従来の `Area/Code` 直下関係と `Warning/@type` 完全一致の回帰テストも維持する。
...
6. 直下に `Status` のみを持ち、その値が完全一致で `発表警報・注意報はなし` の Kind は、Name・Code を要求せず `kindType: 'no_warning'` の正常結果 1 件として保持される。構造不正・対象地域外・空配列に縮退させず、存在しない通常 Kind の項目を補わない。C3 は判別子で通常 Kind と区別できる。
...
- **C3**: `parseWarningTelegram` を唯一の種別固有 XML 解釈器として再利用し、個別 VPWW55–61 と VPWS50 を要素時刻・電文適用範囲に基づき積み上げる。`kindType === 'warning'` へ型を絞り込んだ後にだけ `name`、`code`、`dateTime`、全 `properties`、`LastKind`、`Addition` を参照する。`kindType === 'no_warning'` は対象区域について「発表警報・注意報はなし」と明示する正常入力であり、対象地域外やパース失敗、`kinds: []` と同一視しない。C3 が電文の適用範囲と集約／個別の規則に従って現況への反映を決め、C2 の段階では現況を上書きしない。市町村の解除 `00` と上位集約の解除対象種別の規則も混同しない。
...
本改訂で追加された 3 件の検収指摘について、Issue #12 の製造を止める未確認事項はない。3、4 は後続 Issue の設計開始前に解消する。
