...
- 重複通知の検出力を落とさないため、1件目到着後に追加待機(100ms)を置いてから unmount する。従来の固定100ms待機は「描画後100ms以内の重複」を検出できたので、1件目到着後100msの窓とすることで、1件目の到着時刻によらず従来と同等以上の窓を確保する(当初50msとしていたが、1件目が早く届くと窓が従来より短くなるとのPR #220レビュー指摘により改訂)。追加待機の不足は偽陰性(重複の見逃し)方向にしか働かず、偽陽性の失敗は生まない。
...
- 257行目の固定待機を次の順に置き換える。
  ...
  2. `await new Promise((resolve) => setTimeout(resolve, 100));`(重複検出窓。数値の意図を日本語コメントで残す)
