...
更新日: 2026-09-19（集計欄を廃止）

**運用注記**: この下書きは #147 までのEpic単位の記録であり、#148 以降のIssueは追加しない。以降のIssue・件数は GitHub を正とする。
...
- **[G9. 各パネル共通：取得状態表示（取得中/取得できません/一部未確認）の実装](https://github.com/BlueKurage119/wx-viewer-poc/issues/60)**
  **クローズ（独立製造単位としない）**。常時あるべき情報の取得中/失敗表示・常時とは限らない情報のカード非表示・staleの前回値表示はG1へ、部分欠測は各パネルへ、取得異常の告知はH5へ引き継ぐ。
...
- **[H5. 端末内診断による気象データ更新停止の静的表示](https://github.com/BlueKurage119/wx-viewer-poc/issues/211)**
  端末内で鮮度を診断し通知領域に静的表示。確認・ブザーなし、解消で自動消去。全端末モードで表示し、K端末ではD7通知との重複を許容。
  依存: H2, A。着手前判断: 情報種別ごとの閾値・文言/位置・既存メタ情報で判定可能か。
  管理ID: 固有の設計判断のみ。
