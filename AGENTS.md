...
1. `main` へ直接コミットしない。
2. 明確な指示なしに PR をマージしない。
3. 明確な指示なしに、既存ファイルの削除、大幅な構造変更、またはマシンに波及する破壊的操作をしない。
4. `docs/basic-design.md` の【設計案】・【未確定】を、無断で確定事項として扱わない。
5. 仕様が曖昧または矛盾するときは、勝手に補完せず、実装前にユーザーへ確認する。調査、計画の提示、承認は実装開始の許可ではない。
6. コミット前に `npm run lint`、`npm run typecheck`、`npm run format:check`、対象 workspace のテストを実行し、成功を確認する。マージ後は `git merge-base --is-ancestor <マージコミット> origin/main` が真であることを確認する。
