...
test('1. 本番 migration をすべて適用すると全テーブルが存在し、__schema_migrations の件数が一致する', () => {
  ...
  try {
    ...
    assert.equal(expectedSqlFiles.length, 8);
    assert.equal(context.migrationSummary.appliedVersions.length, 8);
...
test('8. migration を2回適用しても再実行されない', () => {
  ...
  try {
    ...
    assert.equal(context1.migrationSummary.appliedVersions.length, 8);
    context1.close();
...
test('9. migration ファイル内に BEGIN / COMMIT / ROLLBACK が含まれない', () => {
  ...
  assert.equal(sqlFiles.length, 8, '8 migration files should exist');
