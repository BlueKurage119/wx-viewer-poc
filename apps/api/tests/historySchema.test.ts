...
test('1. 本番 migration をすべて適用すると fetch_attempt / telegram_reception / telegram_reception_area が存在し、適用件数が 10 件と一致する', () => {
  ...
  try {
    ...
    assert.equal(expectedSqlFiles.length, 10);
    assert.equal(context.migrationSummary.appliedVersions.length, 10);
...
test('12. migration を 2 回適用しても再実行されない（appliedVersions が空配列）', () => {
  ...
  try {
    ...
    assert.equal(context1.migrationSummary.appliedVersions.length, 10);
    context1.close();
