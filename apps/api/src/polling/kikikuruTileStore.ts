...
export class KikikuruTileStore {
  ...
  async deleteTileIfUnreferenced(
    ...
    const referenced = connection
      .prepare('SELECT 1 FROM risk_tile WHERE file_path = ? LIMIT 1')
      .get(relativePath);
  ...
  async cleanLayerUnreferencedTiles(
    connection: DatabaseConnection,
    layer: KikikuruLayer,
  ): Promise<void> {
    const rows = connection
      .prepare(
        `
        SELECT rt.file_path FROM risk_tile rt
        JOIN risk_frame rf ON rt.frame_id = rf.id
        JOIN risk_snapshot rs ON rf.snapshot_id = rs.id
        WHERE rs.layer = ?
      `,
      )
      .all(layer) as { file_path: string }[];
    ...
    const layerDir = path.join(this.cacheRoot, 'risk', layer);
    if (!fs.existsSync(layerDir)) {
      return;
    }

    await this.scanAndClean(layerDir, validPaths);
  }
  ...
  cleanOrphanAndTempFiles(connection: DatabaseConnection): void {
    ...
    const riskDir = path.join(this.cacheRoot, 'risk');
    if (!fs.existsSync(riskDir)) {
      return;
    }

    this.scanAndCleanSync(riskDir, validPaths);
  }
